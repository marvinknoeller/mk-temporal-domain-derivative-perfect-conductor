...
for ii = 1 : 3
    if ii == 1
        grid_folder = 'Example3Grids/grids0';
        exact_file  = 'sawn_cube2.msh';
        iter = 13;
    elseif ii == 2
        grid_folder = 'Example3Grids/grids15';
        exact_file  = 'sawn_cube2.msh';
        iter = 11;
    else
        grid_folder = 'Example3Grids/grids30';
        exact_file  = 'sawn_cube2.msh';
        iter = 12;
    end
    ...
    sharp = sharp_edges(nodE, elE, c, angTol);       % creases of the exact object
    %% ---------- figure ----------
    figure('Color', 'w', 'Position', [80 80 1500 900]);
    trisurf(elE, nodE(:,1), nodE(:,2), nodE(:,3), 'FaceColor', [.55 .65 .85], ...
        'FaceAlpha', .12, 'EdgeColor', 'none', 'FaceLighting', 'none');
    hold on
    trisurf(elR, nodR(:,1), nodR(:,2), nodR(:,3), 'FaceVertexCData', d, ...
        'FaceColor', 'interp', 'EdgeColor', 'none', 'FaceLighting', 'none');
    plot_segs(nodE(sharp(:,1),:), nodE(sharp(:,2),:), 'k-', 'LineWidth', 3);
    hold off
    axis equal vis3d
    axis off
    colormap(bwr(1e3));
    ...
    cb.LineWidth = 2.5;
    exportgraphics(gcf,fullfile('plots',strcat('rec3',num2str(ii),'.png')),'Resolution',300)
    close all
end
%% ---------- local functions ----------
...
function E = sharp_edges(pos, tri, c, angTol)
% mesh edges where the two adjacent triangles meet at more than angTol degrees
n  = outward_normals(pos, tri, c);
[E, ~, ic] = unique(sort([tri(:,[1 2]); tri(:,[2 3]); tri(:,[3 1])], 2), 'rows');
t  = repmat((1:size(tri,1)).', 3, 1);
t1 = accumarray(ic, t, [], @min);
t2 = accumarray(ic, t, [], @max);
E  = E(sum(n(t1,:) .* n(t2,:), 2) < cosd(angTol), :);
end
